...
from pyramid3dunet.unet3d.utils import get_logger, get_class
...
## added by guoqing
## usually, the mask is a thinned prediction, so that we can focus on the parts.
## this solution lead to a problem: ideally, coarse segmentation should cover all the label.
class MaskedSliceBuilder(SliceBuilder):
    """
    Filter patches containing more than `1 - threshold` of ignore_index label
    """

    def __init__(self, raw_dataset, label_dataset, weight_dataset, patch_shape, stride_shape, mask=None, ignore_index=(0,),
        ...
        def masked_patch(idx):
            patch = np.copy(mask[idx])
            for ii in ignore_index:
                patch[patch == ii] = 0
            non_ignore_counts = np.count_nonzero(patch != 0)
            non_ignore_counts = non_ignore_counts / patch.size
            return non_ignore_counts > threshold

        # zipped_slices = zip(self.raw_slices, self.label_slices)
        # ignore slices containing too much ignore_index
        self._raw_slices = list(filter(masked_patch, self.raw_slices))
        if self.label_slices is not None:
            self._label_slices = list(filter(masked_patch, self.label_slices))
            assert len(self._raw_slices) == len(self._label_slices)
        
def _loader_classes(class_name):
    modules = [
        'pyramid3dunet.datasets.nii',
        'pyramid3dunet.datasets.img',
        'pyramid3dunet.datasets.utils'
    ]
...
def get_train_loaders(config):
    ...
    return {
        ...
        'train_file_paths':[td.file_path for td in train_datasets]
    }
