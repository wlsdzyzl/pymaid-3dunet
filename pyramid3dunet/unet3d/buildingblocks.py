...
from pyramid3dunet.unet3d.se import ChannelSELayer3D, ChannelSpatialSELayer3D, SpatialSELayer3D
...
class CombineLayer(nn.Module):
    ...
    def forward(self, x):
        assert isinstance(x, list) and len(x) > 2, 'the input of combine layer should be a list whose length is larger than 1'
        final_x = x[-1]
        for tmp_x in x[:-1]:
            final_x = self.joining(self.upsampling(final_x, tmp_x), final_x)
        # print(final_x.shape, final_x)
        return self.final_conv(final_x)
    @staticmethod
    def _joining(encoder_features, x, concat):
        if concat:
            return torch.cat((encoder_features, x), dim=1)
