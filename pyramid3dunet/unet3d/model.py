...
from pyramid3dunet.unet3d.buildingblocks import DoubleConv, ResNetBlock, ResNetBlockSE, \
    create_decoders, create_encoders, CombineLayer
from pyramid3dunet.unet3d.utils import get_class, number_of_features_per_level
...
class AbstractMQUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        self.decoders = create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups,
                                        is3d)
        # final combine layer
        self.combine_layer = CombineLayer(len(f_maps), out_channels)
        # in the last layer a 1×1 convolution reduces the number of output channels to the number of labels
        if is3d:
            self.final_convs = nn.ModuleList([nn.Conv3d(fm, out_channels, 1) for fm in reversed(f_maps)])
        else:
            self.final_convs = nn.ModuleList([nn.Conv2d(fm, out_channels, 1) for fm in reversed(f_maps)])

        self.final_res_block = nn.ModuleList([ResNetBlock(fm, fm, is3d=is3d) for fm in reversed(f_maps)])
    ...
    def forward(self, x):
        ...
        for (i, (decoder, encoder_features)) in enumerate(zip(self.decoders, encoders_features)):
            ...
            # of the previous decoder
            # print(i)
            res_x.append(self.final_convs[i]( self.final_res_block[i](x) ))
            x = decoder(encoder_features, x)
            
        res_x.append(self.final_convs[-1](x))
        # add combine layer
        res_x.append(self.combine_layer(res_x))
        # print(res_x[-1])
        # apply final_activation (i.e. Sigmoid or Softmax) only during prediction.
        # During training the network outputs logits
        if not self.training and self.final_activation is not None:
            for i in range(len(res_x)):
                res_x[i] = self.final_activation(res_x[i])
...
## adopted from https://github.com/Dawn90/V-Net.pytorch/blob/master/vnet.py
...
class InputTransition(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('after batch_normalization: ', out.shape)
        out = self.relu1(out)
        return out
...
class OutputTransition(nn.Module):
    def __init__(self, inChans, elu, nll):
        ...
        self.conv1 = nn.Conv3d(inChans, inChans // 2, kernel_size=5, padding=2)
        self.bn1 = ContBatchNorm3d(inChans // 2)
        self.conv2 = nn.Conv3d(inChans // 2, 1, kernel_size=1)
...
class MQVNet(nn.Module):
    ...
    def __init__(self, elu=True, nll=False, final_sigmoid = True, is_segmentation=True,  **kwargs):
        ...
        self.down_tr128 = DownTransition(64, 3, elu, dropout=True)
        self.up_tr128 = UpTransition(128, 128, 2, elu, dropout=True)
        self.out_tr128 = OutputTransition(128, elu, nll)
        self.up_tr64 = UpTransition(128, 64, 1, elu)
        self.out_tr64 = OutputTransition(64, elu, nll)
        self.up_tr32 = UpTransition(64, 32, 1, elu)
        self.out_tr32 = OutputTransition(32, elu, nll)
        if is_segmentation:
            # semantic segmentation problem
            if final_sigmoid:
                self.final_activation = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        out128 = self.down_tr128(out64)
        out = self.up_tr128(out128, out64)
        output128 = self.out_tr128(out)
        out = self.up_tr64(out, out32)
        output64 = self.out_tr64(out)
        out = self.up_tr32(out, out16)
        output32 = self.out_tr32(out)
        return [output128, output64, output32]
    
def get_model(model_config):
    model_class = get_class(model_config['name'], modules=[
        'pyramid3dunet.unet3d.model'
    ])
